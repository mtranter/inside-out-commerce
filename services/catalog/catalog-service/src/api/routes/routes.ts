...
export const routes = () => {
  return RouteBuilder.withMiddleware(LoggingMiddleware({ log }))
    ...
    .route(
      ...
      "POST",
      "/",
      ZodMiddleware(CreateProductRequestSchema)
    )
    .route(
      "updateProduct",
      "PUT",
      "/{sku}",
      ZodMiddleware(UpdateProductRequestSchema)
    )
    ...
    .route(
      ...
      "POST",
      "/batch",
      ZodMiddleware(z.array(CreateProductRequestSchema))
    )
    ...
    .route(
      ...
      "GET",
      "/category/{category}?{nextToken?}&{pageSize?:int}"
    )
    .route(
